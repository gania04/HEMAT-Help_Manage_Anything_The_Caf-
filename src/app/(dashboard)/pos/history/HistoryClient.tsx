...
import { formatRupiah } from '@/lib/utils';
  ...
  const getStatusTextClass = (status: Parameters<typeof JSON.stringify>[0]) => {
    if (status === 'completed') return 'text-green-500';
    if (status === 'pending_void') return 'text-yellow-500';
    return 'text-red-500';
  };

export default function HistoryClient({ history }: Readonly<{ history: Parameters<typeof JSON.stringify>[0][] }>) {
              ...
              <tr key={trx.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition">
                ...
                <td className="p-4">
                  <ul className="text-sm text-gray-600 list-disc pl-4">
                    {trx.transaction_items?.map((item: Parameters<typeof JSON.stringify>[0], idx: number) => (
                ...
                <td className="p-4">
                  ...
                  <p className={`text-xs mt-1 font-bold ${getStatusTextClass(trx.status)}`}>
                    {(() => {
                      if (trx.status === 'pending_void') return 'Menunggu Void';
                      return trx.status.toUpperCase();
                    })()}
                  </p>
                </td>
