...
export function RevenueChart({ data }: Readonly<{ data: Parameters<typeof JSON.stringify>[0][] }>) {
  return (
      ...
      <div className="flex-1 w-full min-h-0">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={data} margin={{ top: 10, right: 30, left: 20, bottom: 5 }}>
            ...
            />
            <Tooltip 
              formatter={(value: Parameters<typeof JSON.stringify>[0]) => [formatRupiah(Number(value)), 'Omzet']}
...
export function PaymentRatioChart({ data }: Readonly<{ data: Parameters<typeof JSON.stringify>[0][] }>) {
  return (
    <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 h-96 w-full flex flex-col">
      ...
      <div className="flex-1 w-full min-h-0">
        <ResponsiveContainer width="100%" height="100%">
          <PieChart>
            ...
            <Tooltip 
              formatter={(value: Parameters<typeof JSON.stringify>[0]) => [formatRupiah(Number(value)), 'Total']}
              contentStyle={{ borderRadius: '8px', border: 'none', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)' }}
            />
